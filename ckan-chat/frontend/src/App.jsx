...
${doveFilter}  FILTER(${filterStr})
  ...
  async function doValidate(url, datasetTitle = "", datasetDescription = "", csvText = null) {
    ...
    if (r.status === 422 || r.status === 413) {
      const data = await r.json();
      throw new Error(data.error || (r.status === 413 ? "File troppo grande" : "Formato non CSV"));
    }
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
...
SELECT ?ipaCode WHERE {
  ...
  async function validateFromCard(url, datasetTitle, publisher = "", datasetUri = "", datasetDescription = "") {
    ...
    try {
      ...
      let csvText = null;
      try {
        // Prima richiesta HEAD per controllare dimensione senza scaricare
        ...
        try {
          ...
          // HEAD fallita (CORS o rete) — procedi con GET ma controlla dopo
        }
        ...
        if (csvRes.ok) {
          ...
          if (!isHtml || urlIsCsv) {
            // Leggi il body progressivamente per bloccare file grandi senza scaricarli tutti
    ...
    <div className="app">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <div id="sidebar-tools" className="sidebar-section">
          ...
          <a className="sidebar-plain-link" href="https://cruscotto-italia-mcp.agid.workers.dev/" target="_blank" rel="noopener noreferrer">
            Cruscotto Italia MCP Server <span className="plain-tag plain-tag-agid">AgID</span>
          </a>
          ...
          <a className="sidebar-plain-link" href="https://github.com/agID/ckan-mcp-server" target="_blank" rel="noopener noreferrer">
            CKAN MCP Server <span className="plain-tag plain-tag-ondata-agid">OnData/AgID</span>
          </a>
        </div>
