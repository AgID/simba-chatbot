...
const MAX_CSV_BYTES = Number(process.env.MAX_CSV_BYTES) || 20 * 1024 * 1024; // 20MB default
...
app.post("/api/intent", strictLimiter, async (req, res) => {
  ...
  // Whitelist acronimi PA/tecnici legittimi
  const PA_ACRONYMS = new Set(["gtfs","dcat","ckan","sparql","istat","agid","anpr","spid","pnrr",
    "pnrr","dgps","cnipa","aipa","pago","suap","sdi","foia","mef","pcm","mit","miur","mur",
    "rdf","ttl","owl","skos","json","csv","xml","http","html","css","sql","api","url","uri",
    "uuid","ipa","pec","cig","cup","cts","tpl","gps","gpx","kml","wfs","wms","wcs","wmts",
    "hvd","ndc","pdnd","gdpr","nis","cie","cns"]);
  ...
  try {
    ...
    if (!csv_text) try {
      ...
      if (csvResp.ok) {
        ...
        const contentLength = parseInt(csvResp.headers.get("content-length") || "0", 10);
        if (contentLength > MAX_CSV_BYTES) {
          const sizeMB = (contentLength / 1024 / 1024).toFixed(1);
          const limitMB = (MAX_CSV_BYTES / 1024 / 1024).toFixed(0);
          return res.status(413).json({
            error: `Il file CSV è troppo grande per la validazione online (${sizeMB} MB). Il limite è ${limitMB} MB. Scarica il file e usa il validatore batch: https://github.com/piersoft/opendata-pa-quality-audit`
          });
        }
        csv_text = await csvResp.text();
        // Verifica anche dopo il download (Content-Length potrebbe mancare)
        if (csv_text.length > MAX_CSV_BYTES) {
          const sizeMB = (csv_text.length / 1024 / 1024).toFixed(1);
          const limitMB = (MAX_CSV_BYTES / 1024 / 1024).toFixed(0);
          return res.status(413).json({
            error: `Il file CSV è troppo grande per la validazione online (${sizeMB} MB). Il limite è ${limitMB} MB. Scarica il file e usa il validatore batch: https://github.com/piersoft/opendata-pa-quality-audit`
          });
...
app.get("/api/preview-csv", async (req, res) => {
  ...
  try {
    ...
    const text = (await r.text()).slice(0, PREVIEW_MAX_BYTES);
    const received = text.length;
    console.log(`[preview] body=${received} chars, inizio=${JSON.stringify(text.slice(0,60))}`);
