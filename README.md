...
Il motore è il `worker.js`, che riconosce automaticamente 30+ ontologie basandosi su un corpus di **503 fixture PA italiane reali** (aggiornato automaticamente).

**Aggiornamento del motore RDF**: il `worker.js` è bundled nell'immagine Docker di `rdf-mcp` (committato nella repo, copiato al build). L'aggiornamento avviene tramite GitHub Action che pusha la nuova versione del `worker.js` nella repo. Per applicare l'aggiornamento sul server:

```bash
docker compose -f docker-compose-full.yml build rdf-mcp && docker compose -f docker-compose-full.yml up -d rdf-mcp
```
...
```bash
git clone https://github.com/AgID/simba-chatbot.git
cd simba-chatbot
```
...
### 3. Avvia i container (MAIN PROCEDURE)
...
**Con Ollama eventualmente già installato sul server:**
...
└── rdf-mcp/
    ├── server.js                 ← adapter worker.js CSV-to-RDF (worker.js bundled)
    └── Dockerfile
```
...
**rdf-mcp non parte** → Verifica i log: `docker compose logs rdf-mcp`. Se il problema è il caricamento del `worker.js`, controlla che il file sia presente in `rdf-mcp/worker.js` e fai un rebuild: `docker compose -f docker-compose-full.yml build rdf-mcp && docker compose -f docker-compose-full.yml up -d rdf-mcp`.
...
Sviluppato da [@piersoft](https://github.com/piersoft) per [AgID](https://www.agid.gov.it).

## Licenza

AGPL 3.0 — [AgID](https://github.com/AgID)
